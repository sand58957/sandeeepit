...
document.addEventListener('DOMContentLoaded', function () {
  ...
  try {
    ...
    if (accent) preset_change(accent);
  } catch (e) {}
  // Restore sidebar caption
  try {
    var caption = localStorage.getItem('sidebar-caption');
    if (caption) layout_caption_change(caption);
  } catch (e) {}
  // Restore theme layout (LTR/RTL)
  try {
    var direction = localStorage.getItem('theme-direction');
    if (direction) layout_rtl_change(direction);
  } catch (e) {}
  // Restore layout width
  try {
    var width = localStorage.getItem('layout-width');
    if (width) change_box_container(width);
  } catch (e) {}

  // Initialize datatables
  document.querySelectorAll('table[data-table="true"]').forEach(table => {
  ...
  if (layoutResetBtn) {
    layoutResetBtn.addEventListener('click', () => {
      // Remove all layout-related keys from localStorage
      try {
        localStorage.removeItem('theme-mode');
        localStorage.removeItem('accent-color');
        localStorage.removeItem('sidebar-caption');
...
function layout_caption_change(value) {
  // Save sidebar caption to localStorage
  try {
    localStorage.setItem('sidebar-caption', value);
  } catch (e) {}
...
function layout_rtl_change(value) {
  // Save theme layout (LTR/RTL) to localStorage
  try {
    localStorage.setItem('theme-direction', value);
  } catch (e) {}
  const body = document.querySelector('body');
...
function layout_change(layout) {
  ...
  try {
    localStorage.setItem('theme-mode', layout);
  } catch (e) {}
...
function change_box_container(value) {
  // Save layout width to localStorage
  try {
